/* IVA v3 — data store: state, validation, migration (v1/v2→v3), demo seed, snapshots */
"use strict";

const Store = {
  KEY: "iva-data-v3",
  LEGACY_V2_KEY: "iva-data-v2",
  LEGACY_V1_KEY: "iva-vanilla-data",
  ...
  load() {
    ...
    // Migration: v2 (colleague/upstream) → v3, then legacy v1 → v3. Never silently discard real data.
    const v2 = Store.migrateV2();
    if (v2) { Store.state = v2; Store.save(); return Store.state; }
    const v1 = Store.migrateV1();
    if (v1) { Store.state = v1; Store.save(); return Store.state; }
  ...
  /* ------------ v1 → v3 / v2 → v3 migration (no silent data loss) ------------ */
  ...
  migrateV2() {
    try {
      const raw = JSON.parse(localStorage.getItem(Store.LEGACY_V2_KEY));
      if (!raw || typeof raw !== "object") return null;
      if (!Array.isArray(raw.transactions) && !Array.isArray(raw.accounts) && !Array.isArray(raw.budgets)) return null;
      const migrated = Store.sanitize(raw);
      if (!migrated.accounts.length && !migrated.transactions.length && !migrated.budgets.length && !migrated.goals.length && !migrated.debts.length) return null;
      try { localStorage.removeItem(Store.LEGACY_V2_KEY); } catch (e) { }
      return migrated;
    } catch (e) { return null; }
  },
  ...
  migrateV1() {
    let old = null;
    try { old = JSON.parse(localStorage.getItem(Store.LEGACY_V1_KEY)); } catch (e) { return null; }
    ...
    const accounts = (old.accounts || []).map((a, i) => ({
      ...
      balance: Store.num(a.amount), color: /^#[0-9a-f]{6}$/i.test(a.color || "") ? a.color : "#6756e8",
      note: "", cardNumber: ""
    }));
    ...
    const transactions = (old.transactions || []).map((t, i) => {
      ...
      const category = Store._catMap[String(t.category || "")] || (isIncome ? "salary" : "other");
      return {
        ...
        date: U.isoToday(),
        amount: Math.abs(Store.num(t.amount || (t.meta && t.meta.amount))) * (type === "expense" ? -1 : 1),
        note: ""
      };
    });
    ...
    try { localStorage.removeItem(Store.LEGACY_V1_KEY); } catch (e) { }
    return Store.sanitize({ version: 3, transactions, accounts, budgets, goals, debts, notes: [] });
  },

  /* ------------ empty initial state (user starts clean) ------------ */
  seed() {
  ...
  /* ------------ realistic demo / sample data (dates relative to today) ------------ */
  seedDemo() {
    ...
    return Store.sanitize({
      version: 3,
      accounts: [
        { id: acc1, name: U.t("seed.acc1"), type: "bank", balance: 82540000, color: "#6756e8", note: fa ? "دمو" : "demo", cardNumber: "6037991234567893" },
        { id: acc2, name: U.t("seed.acc2"), type: "card", balance: 38750000, color: "#2f9df4", note: fa ? "دمو" : "demo", cardNumber: "6104331234567890" },
        { id: acc3, name: U.t("seed.acc3"), type: "cash", balance: 5550000, color: "#10bfa4", note: fa ? "دمو" : "demo", cardNumber: "" },
        { id: acc4, name: U.t("seed.acc4"), type: "savings", balance: 163500000, color: "#f1b83f", note: fa ? "دمو" : "demo", cardNumber: "6273531234567890" }
      ],
      transactions: tx,
      budgets: [
      ...
      notes: [
        { id: "n1", text: fa ? "📌 نمونه: قسط هشتم مدرسه را دادم." : "📌 Sample: paid the 8th school installment.", ts: Date.now() - 3600e3, up: 0 },
        { id: "n2", text: fa ? "📌 نمونه: هدف پس‌انداز ژاپن را هر ماه ۵ میلیون تومان بیشتر کنید." : "📌 Sample: add 5M toman per month to the Japan goal.", ts: Date.now() - 7200e3, up: 0 }
      ]
    });
  }
};
